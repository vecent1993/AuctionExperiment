...
class PlayerHandler(WSMessageHandler):
    ...
    def switch_handler(self, msg=None):
        if 'stage' not in self.player:
            ...
            self.env.msg_handler = handler.hs.handlers['Init'].ph(self.env)
            return

        stage = self.player.get('stage', refresh=True).split(':')[0]
        self.close()
        try:
            self.env.msg_handler = handler.hs.handlers[stage].ph(self.env)
            if msg:
                self.env.on_message(json.dumps(msg))
        except:
            self.write_cmd('error', str(traceback.format_exc()))
