...
class PlayerSealedEnglish(playerhandler.PlayerHandler):
    ...
    def get(self, data):
        ...
        if mainstage == 'sealed':
            self.write_cmd('replace', self.render('sealed.html', train=False, round=round,
            ...
            q = None
            self.write_cmd('replace', self.render('english.html', train=False, round=round,
    ...
    def get_timeout(self, name=None):
        ...
        runtime = grouphandler.GroupHandler.get_runtime(tid)
        if runtime:
